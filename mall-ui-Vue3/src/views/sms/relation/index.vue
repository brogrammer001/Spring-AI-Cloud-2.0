    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['sms:relation:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['sms:relation:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="relationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['sms:relation:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['sms:relation:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listRelation, getRelation, delRelation, addRelation, updateRelation } from "@/api/sms/relation"
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除优惠券分类关联编号为"' + _ids + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('sms/relation/export', {
    ...queryParams.value

 ...
 *
 * <p>
 * NOTE: This class overrides {@code NacosMcpGatewayToolCallback} from
 * {@code spring-ai-alibaba-mcp-gateway} (same package & name, project classes win over
 * dependency jars). The upstream version hard-codes a 30s timeout in
 * {@link #getTimeoutDuration()} and builds the MCP client without a request timeout
 * (SDK default is only 20s), which causes
 * {@code "Error: MCP call failed - java.util.concurrent.TimeoutException"}
 * for long-running tool calls. Here the timeout is configurable via
 * {@code mcp.gateway.tool-timeout-seconds} (default 600s) and is also applied to the
 * MCP client's request/initialization timeout.
 * </p>
 */
public class NacosMcpGatewayToolCallback implements ToolCallback {
    ...
    /**
     * Config property key for the tool call timeout, in seconds.
     */
    private static final String TIMEOUT_PROPERTY = "mcp.gateway.tool-timeout-seconds";

    /**
     * Default tool call timeout (seconds) when the property is not set.
     */
    ...
    private String handleMcpStreamProtocol(Map<String, Object> args, McpServerRemoteServiceConfig remoteServerConfig,
        ...
        try {
            ...
            // Create MCP sync client with configurable request/initialization timeout
            // (upstream builds without a timeout, so the SDK default of 20s applies and
            // long-running tool calls fail with TimeoutException)
            Duration timeout = getTimeoutDuration();
    ...
    private Duration getTimeoutDuration() {
        try {
            Environment env = SpringBeanUtils.getInstance().getBean(Environment.class);
            if (env != null) {
                Long seconds = env.getProperty(TIMEOUT_PROPERTY, Long.class, DEFAULT_TIMEOUT_SECONDS);
                if (seconds != null && seconds > 0) {
                    return Duration.ofSeconds(seconds);
                }
            }
        } catch (Exception e) {
            logger.debug("[getTimeoutDuration] Unable to read '{}', using default {}s",
                    TIMEOUT_PROPERTY, DEFAULT_TIMEOUT_SECONDS);
        }
